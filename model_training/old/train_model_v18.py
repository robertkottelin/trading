"""
BTC ML Training -- Iteration 18: FINE-GRAINED DD OPTIMIZATION
================================================================================
Building on v17 (10/10 WF, +3713% annual, -15.2% worst DD, Sharpe 29.1).
Key changes:
  - Fine-grained DD sweep: 0.5%, 0.6%, 0.7%, 0.8%, 0.9%, 1.0%, 1.2%, 1.5%
  - Max concurrent sweep: 10, 12, 14 (more room with tighter DD)
  - Cooldown: 10, 12, 15
  - Same 25 proven models from v17
...
RESULTS_FILE = os.path.join(OUTPUT_DIR, "training_results_v18.txt")
MODEL_DIR = os.path.join(OUTPUT_DIR, "models_v18")

FEE_MAKER_RT = 0.0004
N_SPLITS = 10

# V15 OPTIMIZED PARAMS (proven stable v15-v17)
V15_PARAMS = {
    "up_12_0002": {
...
def main():
    ...
    with open(RESULTS_FILE, "w", encoding="utf-8") as f:
        log(f"BTC ML Training -- Iteration 18: FINE-GRAINED DD OPTIMIZATION", f)
        log(f"{'='*80}", f)
        ...
        log(f"Using v15 Optuna params + v17 model universe (25 models)", f)
        log(f"DD sweep: 0.5% to 1.5% in fine increments", f)
        log(f"Max concurrent: 10, 12, 14", f)
        ...
        # Phase 3: Fine-grained DD sweep
        log(f"\n{'#'*80}", f)
        log(f"  PHASE 3: FINE-GRAINED DD SWEEP", f)
        log(f"{'#'*80}\n", f)
        ...
        sweep_results = {}
        for max_dd in [0.005, 0.006, 0.007, 0.008, 0.009, 0.01, 0.012, 0.015]:
            for cooldown in [10, 12, 15]:
                for max_conc in [10, 12, 14]:
                    for pos_scale in [0.80, 1.0]:
                        ...
                        dd_label = f"{max_dd*1000:.0f}m" if max_dd < 0.01 else f"{max_dd*100:.0f}p"
        ...
        log(f"\n  === COMPARISON vs V17 ===", f)
        log(f"  V17: 10/10 pos, avg +1063.6%, worstDD -15.2%, Sharpe 29.1, annual +3713%", f)
        log(f"  V18: {cfg_data['pos']}/10 pos, avg {cfg_data['avg_net']:+.1%}, "
        ...
        config_path = os.path.join(MODEL_DIR, "production_config_v18.json")
