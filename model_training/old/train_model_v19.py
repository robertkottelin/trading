"""
BTC ML Training -- Iteration 19: PUSH DD + CONCURRENCY + COOLDOWN BOUNDARIES
================================================================================
Building on v18 (10/10 WF, +17505% annual, -13.9% worst DD, Sharpe 35.8).
V18 found: DD=0.5%, cooldown=10, max_concurrent=14 — ALL at sweep boundaries.
Key changes:
  - DD sweep: 0.2%, 0.3%, 0.4%, 0.5% (testing sub-0.5% — continue the trend?)
  - Max concurrent: 14, 16, 18, 20 (testing higher — 14 was at boundary)
  - Cooldown: 6, 8, 10 (testing lower — 10 was at boundary)
  - Position scale: 0.8, 1.0
  - Same 25 proven models from v17/v18
  - Same v15 Optuna params and ensemble decisions
  - Same recent-weighted quality scoring

Phases:
  0: Feature selection (per-target top-100)
  1: Train all models (10-split WF, v15 params)
  2: Quality scoring (recent-weighted)
  3: Boundary-push DD sweep
  4: Final production config

Usage:
  python train_model_v19.py
"""
...
RESULTS_FILE = os.path.join(OUTPUT_DIR, "training_results_v19.txt")
MODEL_DIR = os.path.join(OUTPUT_DIR, "models_v19")

FEE_MAKER_RT = 0.0004
N_SPLITS = 10

# V15 OPTIMIZED PARAMS (proven stable v15-v18)
V15_PARAMS = {
    "up_12_0002": {
...
def main():
    ...
    with open(RESULTS_FILE, "w", encoding="utf-8") as f:
        log(f"BTC ML Training -- Iteration 19: PUSH DD + CONCURRENCY + COOLDOWN BOUNDARIES", f)
        log(f"{'='*80}", f)
        ...
        log(f"Using v15 Optuna params + v17/v18 model universe (25 models)", f)
        log(f"DD sweep: 0.2% to 0.5% (pushing below v18's 0.5%)", f)
        log(f"Max concurrent: 14, 16, 18, 20 (pushing above v18's 14)", f)
        log(f"Cooldown: 6, 8, 10 (pushing below v18's 10)", f)
        ...
        for name, tgt_col, horizon, prob_thresh, top_pct, purge_mult in ALL_MODELS:
            ...
            log(f"  {name:<25s} AUC={np.mean(aucs):.4f}+/-{np.std(aucs):.3f}  "
                ...
                f"t={elapsed:.0f}s{ens_tag}{forced_tag}{tag}", f)
        ...
        # Phase 3: Boundary-push DD sweep
        log(f"\n{'#'*80}", f)
        log(f"  PHASE 3: BOUNDARY-PUSH DD SWEEP", f)
        log(f"{'#'*80}\n", f)
        ...
        sweep_results = {}
        for max_dd in [0.002, 0.003, 0.004, 0.005]:
            for cooldown in [6, 8, 10]:
                for max_conc in [14, 16, 18, 20]:
                    for pos_scale in [0.80, 1.0]:
                        ...
                        dd_label = f"{max_dd*1000:.0f}m"
                        cfg = f"dd{dd_label}_cl{cooldown}_c{max_conc}_s{int(pos_scale*100)}"
                        sweep_results[cfg] = {
        ...
        # Find best configs at different DD thresholds
        ...
        if candidates:
            ...
            if safest:
                ...
                log(f"  Best (Sharpe, DD>-16%): {best_safe[0]}  "
                    f"sharpe={best_safe[1]['sharpe']:+.1f}  avg={best_safe[1]['avg_net']:+.1%}  "
                    f"worstDD={best_safe[1]['worst_dd']:+.1%}", f)

            safest2 = {k: v for k, v in candidates.items() if v["worst_dd"] > -0.12}
            if safest2:
                best_safe2 = sorted(safest2.items(), key=lambda x: -x[1]["sharpe"])[0]
                log(f"  Best (Sharpe, DD>-12%): {best_safe2[0]}  "
                    f"sharpe={best_safe2[1]['sharpe']:+.1f}  avg={best_safe2[1]['avg_net']:+.1%}  "
                    f"worstDD={best_safe2[1]['worst_dd']:+.1%}", f)

            safest3 = {k: v for k, v in candidates.items() if v["worst_dd"] > -0.08}
            if safest3:
                best_safe3 = sorted(safest3.items(), key=lambda x: -x[1]["sharpe"])[0]
                log(f"  Best (Sharpe, DD>-8%): {best_safe3[0]}  "
                    f"sharpe={best_safe3[1]['sharpe']:+.1f}  avg={best_safe3[1]['avg_net']:+.1%}  "
                    f"worstDD={best_safe3[1]['worst_dd']:+.1%}", f)
        ...
        if not safe:
            safe = candidates
        final_cfg = sorted(safe.items(), key=lambda x: -x[1]["sharpe"])[0]

        cfg_name, cfg_data = final_cfg
        max_dd, cooldown, max_conc, pos_scale = cfg_data["params"]

        log(f"  +--------------------------------------------------+", f)
        log(f"  |  FINAL CONFIG: {cfg_name:<33s}|", f)
        log(f"  +--------------------------------------------------+", f)
        ...
        log(f"\n  === COMPARISON vs V18 ===", f)
        log(f"  V18: 10/10 pos, avg +14622.4%, worstDD -13.9%, Sharpe 35.8, annual +17505%", f)
        log(f"  V19: {cfg_data['pos']}/10 pos, avg {cfg_data['avg_net']:+.1%}, "
        ...
        config_path = os.path.join(MODEL_DIR, "production_config_v19.json")
