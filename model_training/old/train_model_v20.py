"""
BTC ML Training -- Iteration 20: NEW TARGET CANDIDATES + OPTIMAL DD CONFIG
================================================================================
Building on v19 (Sharpe 38.1). DD optimization is reaching diminishing returns
(v15-v19: 3%→2%→1%→0.5%→0.2%). This iteration focuses on new alpha:
  - 8 new candidate models from untried targets:
    up_48_0003, up_24_0005, up_36_0005, fav_12_0002, fav_36_0002, fav_36_0005
  - Use best DD configs from v18/v19 (no new DD sweep — already optimized)
  - Same 25 proven models from v17-v19
  - Test if new candidates improve the portfolio

Phases:
  0: Feature selection (per-target top-100)
  1: Train all models — existing 25 + 8 new candidates (10-split WF)
  2: Quality scoring (recent-weighted)
  3: Portfolio comparison: best DD configs with vs without new candidates
...
RESULTS_FILE = os.path.join(OUTPUT_DIR, "training_results_v20.txt")
MODEL_DIR = os.path.join(OUTPUT_DIR, "models_v20")

FEE_MAKER_RT = 0.0004
N_SPLITS = 10

# V15 OPTIMIZED PARAMS (proven stable v15-v19)
V15_PARAMS = {
    "up_12_0002": {
...
# Existing 25 models from v17-v19
EXISTING_MODELS = [
...
# NEW candidate models from untried targets
NEW_CANDIDATES = [
    # 4h 0.3% threshold (higher threshold than existing up_48_0002)
    ("up_48_0003_p35t10",  "target_up_48_0003",  48, 0.35, 0.10, 2),
    ("up_48_0003_p40t10",  "target_up_48_0003",  48, 0.40, 0.10, 2),
    # 2h 0.5% threshold (larger moves)
    ...
    # Favorable (asymmetric risk-reward) — low threshold, should have more signal
    ("fav_12_0002_p35t20", "target_favorable_12_0002", 12, 0.35, 0.20, 2),
    ("fav_12_0002_p40t10", "target_favorable_12_0002", 12, 0.40, 0.10, 2),
    # Favorable 3h — different horizons
    ("fav_36_0002_p35t20", "target_favorable_36_0002", 36, 0.35, 0.20, 2),
    ("fav_36_0005_p35t10", "target_favorable_36_0005", 36, 0.35, 0.10, 2),
    # 1h 1% move (very aggressive, rare signals)
    ("up_12_001_p40t10",   "target_up_12_001",   12, 0.40, 0.10, 2),
]
...
def get_params(model_name):
    ...
    if "fav_12_0002" in model_name:
        return V15_PARAMS["fav_12_0003"]  # closest: same target family
    if "fav_12_0003" in model_name:
        return V15_PARAMS["fav_12_0003"]
    if "fav_12_0005" in model_name:
        return V15_PARAMS["fav_12_0005"]
    if "fav_36_0002" in model_name or "fav_36_0003" in model_name or "fav_36_0005" in model_name:
        return V15_PARAMS["up_36_0003"]  # closest: same horizon
    if "up_48_0003" in model_name:
        return V15_PARAMS["up_48_0002"]  # closest: same horizon
    if "up_24_0005" in model_name:
        return V15_PARAMS["up_24_0003"]  # closest: same horizon, higher threshold
    if "up_36_0005" in model_name:
        return V15_PARAMS["up_36_0003"]  # closest: same horizon, higher threshold
    if "up_12_001" in model_name:
        return V15_PARAMS["up_12_0005"]  # closest: same horizon, higher threshold
...
def get_ens_config(model_name):
    """Map model name to ensemble config. New candidates default to LGB-only."""
    if model_name in FORCE_LGB_ONLY:
        return (False, 1.0)
    # New candidates: try ensemble for fav targets, LGB-only for others
    if "fav_12_0002" in model_name:
        return (True, 0.5)
    if "fav_36_0002" in model_name or "fav_36_0005" in model_name:
        return (True, 0.7)
...
def main():
    ...
    with open(RESULTS_FILE, "w", encoding="utf-8") as f:
        log(f"BTC ML Training -- Iteration 20: NEW TARGET CANDIDATES + OPTIMAL DD CONFIG", f)
        log(f"{'='*80}", f)
        ...
        log(f"New targets: up_48_0003, up_24_0005, up_36_0005, fav_12_0002, fav_36_0002, fav_36_0005, up_12_001", f)
        ...
        targets_for_feat = [
            ...
            ("fav_12_0002", "target_favorable_12_0002"),
            ("fav_12_0003", "target_favorable_12_0003"),
            ("fav_12_0005", "target_favorable_12_0005"),
            ("fav_36_0002", "target_favorable_36_0002"),
        ...
        for tgt_name, tgt_col in targets_for_feat:
            ...
            top5 = imp.head(5)
            tag = " [NEW]" if tgt_name not in ["up_12_0002", "up_12_0003", "up_12_0005",
                    "up_24_0002", "up_24_0003", "up_36_0002", "up_36_0003", "up_48_0002",
                    "fav_12_0003", "fav_12_0005"] else ""
            log(f"  {tgt_name}: top5 = {list(top5['feat'].values)}{tag}", f)

        def get_feats(name, n_feats=100):
            # New targets
            if "fav_12_0002" in name and "fav_12_0002" in feat_cache:
                return feat_cache["fav_12_0002"][:n_feats]
            if "fav_36_0002" in name and "fav_36_0002" in feat_cache:
                return feat_cache["fav_36_0002"][:n_feats]
            if "fav_36_0005" in name and "fav_36_0005" in feat_cache:
                return feat_cache["fav_36_0005"][:n_feats]
            if "up_48_0003" in name and "up_48_0003" in feat_cache:
                return feat_cache["up_48_0003"][:n_feats]
            if "up_24_0005" in name and "up_24_0005" in feat_cache:
                return feat_cache["up_24_0005"][:n_feats]
            if "up_36_0005" in name and "up_36_0005" in feat_cache:
                return feat_cache["up_36_0005"][:n_feats]
            if "up_12_001" in name and "up_12_001" in feat_cache:
                return feat_cache["up_12_001"][:n_feats]
            # Existing targets
            ...
            if "fav_36_0003" in name:
                return feat_cache.get("fav_36_0003", feat_cache.get("up_36_0003", all_feat_cols[:n_feats]))[:n_feats]
        ...
        configs_to_test = [
            (0.002, 10, 14, 1.0, "dd2m_cl10_c14_s100"),   # Best risk-adj from v19
            (0.002, 10, 20, 0.8, "dd2m_cl10_c20_s80"),     # Best Sharpe from v19
            (0.005, 10, 14, 1.0, "dd5m_cl10_c14_s100"),    # Best from v18
        ...
        # Test all configs with expanded portfolio to find best
        best_sharpe = -1
        best_cfg = None
        best_params = None
        all_cfg_results = {}
        ...
        log(f"\n  === COMPARISON vs V19 ===", f)
        log(f"  V19: 10/10 pos, avg +4161.5%, worstDD -17.1%, Sharpe 38.1, annual +7192%", f)
        log(f"  V20: {cfg_data['pos']}/10 pos, avg {cfg_data['avg_net']:+.1%}, "
        ...
        config_path = os.path.join(MODEL_DIR, "production_config_v20.json")
