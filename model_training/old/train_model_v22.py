"""
BTC ML Training -- Iteration 22: OPTUNA FOR 30-MIN TARGETS + DD CONFIG SWEEP
================================================================================
Building on v21 (Sharpe 38.8, 40 models, worst DD -14.8%). The 30-min (up_6)
targets were a goldmine (6/6 passed, AUC 0.67-0.77) but use borrowed 1h params.
V21's Phase 3 showed dd2m_cl10_c16_s100 was a potential sweet spot.

This iteration:
  - Optuna optimization for up_6_0002, up_6_0003, up_6_0005 (dedicated 30-min params)
  - Retrain all 40 models using optimized params for 30-min targets
  - Extended DD config sweep: c10-c24, DD 0.2%/0.3%, to find optimal for 40 models
  - Also optimize up_24_001 and up_48_0005 (borrowed params in v21)

Phases:
  0: Optuna optimization for new target families (up_6, up_24_001, up_48_0005)
  1: Feature selection (per-target top-100)
  2: Train all 40 models (10-split WF, updated params)
  3: Quality scoring (recent-weighted)
  4: Extended DD config sweep (12 configs)
  5: Final production config

Usage:
  python train_model_v22.py
"""
...
RESULTS_FILE = os.path.join(OUTPUT_DIR, "training_results_v22.txt")
MODEL_DIR = os.path.join(OUTPUT_DIR, "models_v22")

FEE_MAKER_RT = 0.0004
N_SPLITS = 10

# V15 OPTIMIZED PARAMS (proven stable v15-v21, for targets already optimized)
V15_PARAMS = {
    "up_12_0002": {
...
# Targets to optimize with Optuna (new in v22)
# Use 40 trials with feature-selected 100 features (not all 309!) to avoid stuck trials
OPTUNA_TARGETS = [
    ("up_6_0002",  "target_up_6_0002",   6, 40),   # 30-min — highest priority
    ("up_6_0003",  "target_up_6_0003",   6, 40),
    ("up_6_0005",  "target_up_6_0005",   6, 40),
    ("up_24_001",  "target_up_24_001",  24, 40),   # 1% threshold at 2h
    ("up_48_0005", "target_up_48_0005", 48, 40),   # Higher threshold at 4h
]
...
def main():
    ...
    with open(RESULTS_FILE, "w", encoding="utf-8") as f:
        log(f"BTC ML Training -- Iteration 22: OPTUNA FOR 30-MIN TARGETS + DD CONFIG SWEEP", f)
        log(f"{'='*80}", f)
        ...
        log(f"Models: {len(ALL_MODELS)} (retrain all with optimized params)", f)
        log(f"Optuna targets: {[t[0] for t in OPTUNA_TARGETS]}", f)

        # =====================================================================
        # PHASE 0: FEATURE SELECTION (before Optuna — use borrowed params)
        # =====================================================================
        log(f"\n{'#'*80}", f)
        log(f"  PHASE 0: FEATURE SELECTION (per-target top-100, using borrowed params)", f)
        log(f"{'#'*80}\n", f)

        # Borrowed param mapping for initial feature selection
        def get_borrowed_params(model_name):
            """Map model name to borrowed params (pre-Optuna)."""
        ...
        # =====================================================================
        # PHASE 1: OPTUNA OPTIMIZATION (using selected 100 features)
        # =====================================================================
        log(f"\n{'#'*80}", f)
        log(f"  PHASE 1: OPTUNA OPTIMIZATION (100 features, new target families)", f)
        log(f"{'#'*80}\n", f)
        ...
        for tgt_name, tgt_col, horizon, n_trials in OPTUNA_TARGETS:
            ...
            # Use feature-selected 100 features (much faster than all 309)
            opt_feats = get_feats(tgt_name, 100)
            purge = 2 * horizon
            log(f"\n  Optimizing {tgt_name} ({len(opt_feats)} features, {n_trials} trials, horizon={horizon})...", f)

            # Prepare 3 WF splits for optimization (using selected features)
            opt_splits = []
            for s in range(3):
            ...
            # Compare with borrowed params baseline (already computed on same features)
            borrowed_key = {
                "up_6_0002": "up_12_0002", "up_6_0003": "up_12_0003",
                "up_6_0005": "up_12_0005", "up_24_001": "up_24_0003",
                "up_48_0005": "up_48_0002",
            }.get(tgt_name, tgt_name)
            borrowed_p = V15_PARAMS.get(borrowed_key)
            if borrowed_p:
                borrowed_aucs = []
        ...
        def get_params(model_name):
            """Map model name to best available params (v22 Optuna > v15 Optuna > borrowed)."""
            # Check v22 Optuna results first
            for key in optimized_params:
                if key in model_name:
                    return optimized_params[key]
            # V15 params for existing targets
            if "fav_12_0003" in model_name:
                return V15_PARAMS["fav_12_0003"]
            if "fav_12_0005" in model_name:
                return V15_PARAMS["fav_12_0005"]
        ...
        def get_ens_config(model_name):
            ...
            if model_name in FORCE_LGB_ONLY:
                return (False, 1.0)
            # All "up" targets without v15 ensemble: LGB-only
        ...
        # =====================================================================
        # PHASE 2: TRAIN ALL 40 MODELS (10-split WF)
        # =====================================================================
        log(f"\n{'#'*80}", f)
        log(f"  PHASE 2: TRAIN ALL MODELS (10-split WF, v22 params)", f)
        log(f"{'#'*80}\n", f)
        ...
        for name, tgt_col, horizon, prob_thresh, top_pct, purge_mult in ALL_MODELS:
            ...
            log(f"  {name:<25s} AUC={np.mean(aucs):.4f}+/-{np.std(aucs):.3f}  "
                ...
                f"t={elapsed:.0f}s{ens_tag}{forced_tag}{opt_tag}{tag}", f)
        ...
        # Show v21 vs v22 comparison for optimized models
        log(f"\n  === V21 vs V22 AUC COMPARISON (Optuna targets) ===", f)
        v21_aucs = {
            "up_6_0002": 0.6698, "up_6_0003": 0.7141, "up_6_0005": 0.7720,
            "up_24_001": 0.7388, "up_48_0005": 0.6306,
        }
        for tgt_name in optimized_params:
            # Find models matching this target
            for name in selected:
                if tgt_name in name:
                    v22_auc = np.mean([r["auc"] for r in all_results[name].values()])
                    v21_auc = v21_aucs.get(tgt_name, 0)
                    delta = v22_auc - v21_auc
                    log(f"    {name:<25s} v21={v21_auc:.4f} -> v22={v22_auc:.4f} delta={delta:+.4f}", f)
        ...
        for name in sorted(selected):
            ...
            for key in optimized_params:
                if key in name:
                    opt_tag = " [OPT]"
                    break
            log(f"    {name:<25s}  quality={quality[name]:.3f}  weight={weights[name]:.3f}{opt_tag}", f)
        ...
        # =====================================================================
        # PHASE 4: EXTENDED DD CONFIG SWEEP (12 configs)
        # =====================================================================
        log(f"\n{'#'*80}", f)
        log(f"  PHASE 4: EXTENDED DD CONFIG SWEEP (optimized for 40 models)", f)
        log(f"{'#'*80}\n", f)

        configs_to_test = [
            # DD=0.2%, varying concurrent from c10 to c24
            (0.002, 10, 10, 1.0, "dd2m_cl10_c10_s100"),
            (0.002, 10, 12, 1.0, "dd2m_cl10_c12_s100"),
            ...
            # DD=0.2% with scale=0.8 (v21 winner)
            (0.002, 10, 20, 0.8, "dd2m_cl10_c20_s80"),
            (0.002, 10, 16, 0.8, "dd2m_cl10_c16_s80"),
            # DD=0.3% variants (slightly relaxed breaker)
            (0.003, 10, 16, 1.0, "dd3m_cl10_c16_s100"),
            (0.003, 10, 20, 1.0, "dd3m_cl10_c20_s100"),
        ]
        ...
        # Also identify safest and balanced configs
        safest_dd = 0
        safest_cfg = None
        balanced_cfg = None
        balanced_score = -1
        ...
        log(f"\n  === COMPARISON vs V21 ===", f)
        log(f"  V21: 10/10 pos, avg +6767.5%, worstDD -14.8%, Sharpe 38.8, annual +11466%, minSharpe 29.3", f)
        log(f"  V22: {cfg_data['pos']}/10 pos, avg {cfg_data['avg_net']:+.1%}, "
        ...
        # Models that dropped from v21 (if any)
        v21_models = {m[0] for m in ALL_MODELS}
        dropped = v21_models - set(selected)
        if dropped:
            log(f"\n  === MODELS DROPPED (in v21 but failed v22 selection) ===", f)
        ...
        prod_config = {
            ...
            "optimized_targets": list(optimized_params.keys()),
        ...
        for name in selected:
            if name in saved_models:
                ...
                entry = {
                    ...
                    "optuna_optimized": any(key in name for key in optimized_params),
                }
        ...
        # Save Optuna params for future reuse
        optuna_save = {}
        for k, v in optimized_params.items():
            optuna_save[k] = {key: val for key, val in v.items()
                             if not isinstance(val, (np.integer, np.floating))}
            for key in v:
                if isinstance(v[key], (np.integer, np.floating)):
                    optuna_save[k][key] = v[key].item()
                else:
                    optuna_save[k][key] = v[key]

        optuna_path = os.path.join(MODEL_DIR, "optuna_params_v22.json")
        with open(optuna_path, "w") as of:
            json.dump(optuna_save, of, indent=2)
        log(f"  Optuna params saved: {optuna_path}", f)

        config_path = os.path.join(MODEL_DIR, "production_config_v22.json")
